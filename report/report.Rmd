...
The second step of the corpus creation was to transform spoken data into text. So that it will be easier to capture different linguistic phenomena. So first I created a visual studio project to work on the transcription part and also repository in Github to save, keep track and organize my work. The transformer model I used is called Whisper. The annotated code is available in my repository. When transcribing the videos using the Whisper(cite), the coastal videos were almost perfectly transcribe. However, the transcriber could not identify the punctuation correctly and when a new sentences starts, it will sometimes start with capital letters but not always. Therefore, I had to correct the sentences boundaries manually. In contrast, the transformer model did not detect the words produced by speakers of Cerro de Pasco as good as it did for the coastal dialect. This might be due to the differences in pronunciation grammar and lexicon between them.
