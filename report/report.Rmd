...
The second step of this corpus creation was to find a way to transform the spoken data into text. So, for the transcription, I used a transformer model called Whisper.  I created a repository in github to record and organize my work. The annotated code  is in a script called transcriber. 

When I first started using the base model. the result of the transcription was not that accurate because I was using the base model. The medium model, in contrast, was and although it has some mistakes, it did a grant job. Another thing to mention is that when I try to use the large one, my computer could not handle. 
